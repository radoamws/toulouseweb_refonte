<x-layouts.app :seo="$seo">
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div class="mt-6 flex flex-wrap gap-2">
            <a href="/agenda" class="rounded-full px-3 py-1.5 text-sm {{ ! $category ? 'bg-brand-600 text-white' : 'bg-ink-50 text-ink-700 hover:bg-ink-100' }}">Toutes</a>
            @foreach ($categories as $cat)
                <a href="/agenda/{{ $cat->slug }}" class="rounded-full px-3 py-1.5 text-sm {{ $category?->id === $cat->id ? 'bg-brand-600 text-white' : 'bg-ink-50 text-ink-700 hover:bg-ink-100' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
