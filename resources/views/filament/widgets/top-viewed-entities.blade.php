<x-filament-widgets::widget>
    <x-filament::section heading="Top pages vues — 30 derniers jours">
        @php $rows = $this->getRows(); @endphp

        @if (empty($rows))
            <p class="text-sm text-gray-500 dark:text-gray-400">Aucune vue enregistrée sur cette période.</p>
        @else
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 text-left text-xs uppercase tracking-wide text-gray-500 dark:border-gray-700 dark:text-gray-400">
                        ...
                        <th class="py-2 text-right">Vues</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($rows as $row)
