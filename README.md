...
- `php artisan scrape:cinema` — quotidien 5h, une source AlloCiné par salle (24/27).
- `php artisan scrape:events` — quotidien 5h30, une source par site de billetterie/salle (`scraper_sources`, type `agenda`). Une seule source construite pour l'instant (Théâtre de la Cité, `TheatreDeLaCiteDriver`) — vérifiée en direct (30/30 événements réels importés). Seedée via `php artisan db:seed --class=AgendaScraperSourcesSeeder`. Voir `TECHNICAL_DOCUMENTATION.md` §13 pour les ~5 autres sources réelles identifiées mais pas encore construites.
