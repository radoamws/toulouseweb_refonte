...
### Suite et résolution quasi complète (08/09/2026)

Le client a signalé que les fichiers manquants se trouvaient en réalité dans `old/backEnd/public/news/compressed/` (15 837 fichiers) — un sous-dossier de `news/`, absent au moment de l'analyse du 07/09/2026 ci-dessus (probablement ajouté par le client entre-temps, une fois informé du manque). **Aucun correctif de code n'a été nécessaire** : `LegacyImageImporter` utilise déjà un `RecursiveDirectoryIterator` (vérifié directement en PHP, hors du contexte de la commande, avant de conclure) — `news/compressed/` est indexé automatiquement dès lors qu'il existe et contient des fichiers, comme n'importe quel sous-dossier de `news/`. Une fois `images:news` rejoué : **229/233 (98,3 %)** actualités publiées ont désormais une image résolue (contre 23/233 avant), seules 2 restent introuvables (noms de fichiers avec caractères spéciaux/doublons d'extension, ex. `Stage Hockey RR toussaint 2024 recto.webp.webp`) — un résidu négligeable, non traité plus avant. Suite complète rerejouée : 214 passed (625 assertions), aucune régression.

## 24. Vérification finale SEO/GEO, bonnes pratiques, sitemap automatique (07/09/2026, demande client)

Demande client : *"fait une dernière vérification complète SEO/GEO, bonne pratique, que les pages soient légères, sitemap automatique après CRUD dans l'admin... pour que le site soit impeccable en référencement."*

### Bug réel trouvé et corrigé — `sitemap:generate` pouvait omettre des films à l'affiche

En vérifiant `GenerateSitemap` (jamais testé automatiquement jusqu'ici — aucun test n'existait pour cette commande, contrairement à ce qu'on pourrait attendre d'une commande qui ne dépend QUE de la base applicative, donc entièrement testable en sandbox, contrairement à `migrate:*`/`images:*`) : le filtre "film actuellement à l'affiche" **dupliquait** la logique de `CinemaController::currentlyValid()` mais **sans le correctif** déjà appliqué là-bas le 01/09/2026 (bug DATE-vs-DATETIME, voir §13) — comparait `end_date >= now()` (un objet Carbon complet) au lieu de `now()->toDateString()` contre une colonne `DATE` pure. Conséquence : un film dont la dernière séance se termine aujourd'hui pouvait disparaître du sitemap dès la première seconde après minuit, exactement le même bug que celui déjà corrigé côté front, mais jamais reporté ici puisque c'était un second endroit avec une copie indépendante du même code.

**Fix** : extraction de cette logique dans `Screening::scopeCurrentlyValid()` (un seul endroit, réutilisé par `CinemaController` ET `GenerateSitemap` désormais) — élimine le risque de divergence future entre les deux. Test de régression : `GenerateSitemapTest::test_movie_with_screening_ending_today_is_included` (aurait échoué avant ce correctif).

### Sitemap automatique après CRUD admin (nouveau)

Aucun mécanisme n'existait avant ce jour pour régénérer `sitemap.xml` autrement que par le cron quotidien (`Schedule::command('sitemap:generate')->daily()`, voir §11) — un ajout/modif/suppression en admin pouvait donc rester invisible du sitemap jusqu'à 24h.

- `App\Jobs\RegenerateSitemap` (`ShouldQueue`, `ShouldBeUnique`) + `App\Observers\RegeneratesSitemapObserver`, enregistré sur les 8 modèles réellement présents dans le sitemap (`Category`, `Listing`, `EventCategory`, `Event`, `Cinema`, `Movie`, `News`, `Classified` — voir `AppServiceProvider::SITEMAP_MODELS`).
- **En file d'attente, pas en ligne** : `sitemap:generate` parcourt potentiellement des dizaines de milliers de lignes — le faire de façon synchrone bloquerait chaque sauvegarde admin. Traité par `queue:work --stop-when-empty`, déjà planifié chaque minute (décision d'architecture "pas de worker permanent" sur hébergement mutualisé, voir §14) — la régénération arrive donc au plus tard une minute après la sauvegarde.
- **`ShouldBeUnique`** : si plusieurs sauvegardes se succèdent avant le prochain passage de `queue:work`, une seule régénération réelle est traitée (vérifié empiriquement : `Queue::fake()` applique lui-même la déduplication — 3 sauvegardes distinctes ne poussent qu'1 seul job réel, pas 3). Contrairement à `App\Observers\CloudflarePurgeObserver`/`GoogleIndexingObserver` (accumulation manuelle + `terminating()`), aucun service dédié n'est nécessaire ici : le mécanisme natif `ShouldBeUnique` de Laravel suffit, car il n'y a rien à accumuler (une régénération est toujours complète, jamais incrémentale par URL).
- **Volontairement SANS exclusion Movie/Cinema** (contrairement à `GOOGLE_INDEXING_MODELS`) : `scrape:cinema` sauvegarde des centaines de `Movie`/jour, mais `ShouldBeUnique` déduplique déjà — pas de risque de quota comme pour l'API Google Indexing (qui facture par appel HTTP réel, pas par tentative de dispatch).

Tests : `tests/Feature/GenerateSitemapTest.php` (8 tests — pages statiques, contenu publié inclus, brouillons/expirés exclus, le bug DATE ci-dessus, dispatch du job sur sauvegarde/suppression, dédoublonnage réel vérifié). Suite complète : voir résultat après cette section.

## 25. Audit SEO/GEO/bonnes pratiques/performance final (08/09/2026, demande client)

Demande client : *"fait une dernière vérification complète SEO/GEO, bonne pratique, que les pages soient légères... pour que le site soit impeccable en référencement."* Audit exhaustif (meta tags, structured data, GEO, poids des pages, accessibilité, robots.txt) fait en lecture seule contre le site réellement démarré (`curl` en direct, pas seulement lecture de code), puis correctifs appliqués pour chaque bug réel trouvé.

**Dimensions vérifiées OK, aucun correctif nécessaire** : poids des pages (bundle compilé ~58 Ko gzip total, purge Tailwind v4 complète via `@source`), pagination sur les 5 contrôleurs de listing publics, eager-loading vs. usage réel des relations dans les vues (aucun N+1 trouvé), `alt` sur toutes les images, `loading="lazy"` appliqué correctement (jamais sur les visuels au-dessus de la ligne de flottaison), maillage interne présent sur les 5 types de fiche détaillée, exactitude de `llms.txt`, robots.txt (pas de sur-blocage ni de sous-blocage trouvé).

**Bugs réels trouvés et corrigés :**

1. **La home n'utilisait jamais son contenu SEO migré** (le plus impactant) — `HomeController`/`ContactController` interrogent `Page::where('key', 'home'|'contact')` (code déjà correct), mais `MigrateSeo::findOrCreatePage()` créait ces pages avec la clé générique `seo-menu-{slug}` sur une base fraîchement migrée (le commentaire historique du code supposait à tort qu'une Page 'home' existerait déjà, créée par `DemoContentSeeder` — qui ne tourne jamais en production). Résultat confirmé en direct : `/` servait le titre générique du layout au lieu du vrai contenu migré ("ToulouseWeb.com | Sorties, Agenda, Restaurants & Commerces à Toulouse..."). **Fix** : `MigrateSeo::KEY_OVERRIDES` (`accueil` → `home`, `contact` → `contact`), idempotent et auto-réparant (une Page déjà migrée avec l'ancienne clé générique est corrigée au prochain passage de la commande).
2. **`/agenda`, `/annonces`, `/cinema` n'utilisaient jamais leur page "menu" migrée** (seul `/annuaire` avait ce repli, implémenté à l'origine). Les pages `seo-menu-agenda`/`seo-menu-annonces`/`seo-menu-cinema` existent en base avec un vrai contenu migré, jamais lues. **Fix** : `EventController`/`ClassifiedController`/`CinemaController` répliquent désormais le repli déjà utilisé par `ListingController::index()`. `/actualites` n'a, lui, jamais eu de page "menu" legacy équivalente (vérifié) — repli final hardcodé dans `NewsController` plutôt que de laisser cette page sur le générique.
3. **Image OG par défaut cassée (404)** — `public/images/og-default.jpg` n'a jamais existé ; confirmé activement utilisée (`SiteSetting.default_og_image_url` vide en base réelle). Chaque page sans image dédiée partageait un aperçu de lien cassé sur les réseaux sociaux. **Fix** : repli sur `branding/toulouseweb-icon.png` (icône de marque déjà en dépôt) — un vrai visuel, mais 180×180 est loin du format bannière 1200×630 optimal ; **reste à fournir par le client** un vrai visuel OG dédié.
4. **`MovieTheater` JSON-LD `url` invalide** sur 26/28 salles réelles — fragment de requête AlloCiné brut (`salle_gen_csalle=P0071.html`), pas une URI absolue, fait échouer la validation Rich Results Google. **Fix** : utilise la page canonique de la salle elle-même (toujours valide) plutôt que `external_url`.
5. **`LocalBusiness`/`Restaurant` JSON-LD `telephone` corrompu** sur 163/2978 fiches annuaire (5,5 %) — `phone` legacy mélangeant parfois téléphone/email/HTML brut (`"Tel: 06 07 50 52 98<br>Mail: info@..."`), rendu tel quel dans le JSON-LD, le lien `tel:` (URI invalide) ET le texte visible (le `<br>` littéral s'affichait comme texte sur la page, pas comme un retour à la ligne, à cause de l'échappement Blade). **Fix** : `Listing::cleanPhone()` (accesseur, `phone` reste inchangé en base pour que l'admin voie/corrige la vraie valeur) — coupe à la première balise `<br>` ou au premier marqueur email, puis retire les labels non numériques en tête ("Tel:", "Journée:"...). Meilleur effort assumé (données trop hétérogènes pour un fix garanti à 100 %), retourne `null` plutôt qu'une valeur fausse.
6. **La home n'avait aucun `<h1>`** — chaque section utilise un `<h2>` (`x-ui.section-heading`), le titre de slide du hero un simple `<p>`. **Fix** : `<h1 class="sr-only">` (même pattern déjà utilisé pour le lien d'évitement du layout) — signal SEO/accessibilité sans dicter de refonte visuelle non prévue ici.

**Améliorations mineures appliquées** (cheap, cumulatif avec le pass "impeccable") :
- `SeoResolverService` : troncature titre/description recodée avec `preserveWords: true` et `'…'` (coupait auparavant en plein milieu d'un mot sans aucune indication, ex. "...Commerces à" au lieu de "...Commerces à Toulouse").
- `twitter:image` ajouté (carte `summary_large_image` déclarée sans image dédiée jusqu'ici).
- `WebSite`/`SearchAction` JSON-LD ajouté sur toutes les pages (condition pour l'éligibilité "sitelinks search box" Google) — cible l'annuaire (plus gros volume de contenu), aucune recherche unifiée n'existant sur tout le site.
- `NewsArticle` JSON-LD : `description` passée par `strip_tags()` (un éditeur peut coller du HTML dans le simple `TextInput` `excerpt`, confirmé en direct) ; `publisher` ajouté (propriété recommandée par Google pour l'éligibilité complète aux résultats enrichis Article).
- `X-Robots-Tag: noindex` ajouté sur `/admin/*` (défense en profondeur — `robots.txt` bloque déjà l'exploration mais n'empêche pas une indexation d'URL nue si un lien externe y pointait).
...
## 29. Nettoyage du HTML/entités legacy dans les champs texte (11/09/2026, demande client)

Signalé par le client avec une capture d'écran d'une fiche annuaire réelle : *"pour les anciens annuaires, les textes étaient en HTML avec balisage [dans] la base"* — la fiche affichait littéralement `<p>- D&eacute;pannage - R&eacute;paration<br />...</p>` au lieu d'un texte lisible.

**Cause** : ces colonnes viennent d'un éditeur WYSIWYG legacy qui stockait directement du HTML + entités HTML, alors qu'elles sont rendues ici comme du texte : `Listing.description`/`Event.description` via `{!! nl2br(e($valeur)) !!}` (`e()` échappe les caractères spéciaux mais ne décode PAS les entités HTML — `&eacute;` reste "&eacute;" à l'écran, un vrai `<br />` original devient le texte littéral une fois échappé), `Listing.short_description` via `{{ }}` simple (même souci). Portée réelle mesurée en base : `listings.description` (1 123 lignes), `listings.short_description` (251), `events.description` (4 392), `news.excerpt` (6 082) — soit environ 11 800 lignes affectées au total. `Classified.description` : 0 ligne affectée (vérifié, jamais eu ce problème). `News.body` : non concerné — déjà rendu en HTML brut non échappé (`{!! $news->body !!}`), les entités s'y décodent nativement dans le navigateur.

**Choix : corriger en base plutôt que dans chaque vue** (le client proposait les deux options) — un seul passage bénéficie à la fois à l'affichage public, aux meta-descriptions SEO (`SeoResolverService::generateDescription()` utilise déjà ces mêmes colonnes) et à l'admin (qui affichait, lui aussi, du HTML brut dans un simple champ texte). Contrairement à `Listing::cleanPhone()` (§25 — données réellement ambiguës, gardées en base pour relecture admin), il n'y a ici aucune ambiguïté : personne ne veut de balises/entités littérales dans un champ texte, une réécriture directe est donc appropriée.

**Commande** : `php artisan content:clean-legacy-html` (`--dry-run` disponible) — convertit `<br>`/les sauts de paragraphe en vrais retours à la ligne AVANT de retirer les balises (sinon une liste à puces "- A -<br>- B -" recollerait en "- A - - B -"), décode les entités HTML (`html_entity_decode`), puis normalise les espaces/retours à la ligne superflus laissés par le nettoyage.

**⚠️ Point de vigilance traité explicitement** : la commande écrit via le query builder (`DB::table(...)->update(...)`), JAMAIS via Eloquent `->save()` — sur ~11 800 lignes réelles, un `save()` en boucle aurait déclenché `CloudflarePurgeObserver`/`GoogleIndexingObserver`/`RegeneratesSitemapObserver` des milliers de fois (purge Cloudflare en rafale, quota d'indexation Google de 200/jour épuisé instantanément, régénérations de sitemap en boucle) pour une simple correction de texte sans rapport avec un vrai changement de contenu public. Vérifié par un test dédié (`Http::fake()` + Cloudflare activé) : aucun appel HTTP sortant déclenché par la commande.
...
## 33. ⚠️ Règle Cloudflare "Cache Everything" héritée de l'ancien site — sessions/CSRF cassés (11/09/2026)

Découvert en testant les correctifs du §32 : `/contact` répondait `cf-cache-status: HIT` malgré `Cache-Control: no-cache, private` envoyé par le serveur d'origine (vérifié en direct via SSH, en contournant Cloudflare) — et se re-cachait quasi instantanément après une purge manuelle. Cloudflare retire systématiquement l'en-tête `Set-Cookie` d'une réponse mise en cache : un visiteur tombant sur une version cachée de `/contact` recevait la page sans jeton de session/CSRF valide, et son formulaire échouait avec "Page Expired" (419) — reproduit et confirmé en direct.

**Cause** : une règle "Cache Rules" (successeur des Page Rules, `phase: http_request_cache_settings`) nommée *"Cache Everything [Template]"*, `expression: "true"` (correspond à TOUTE requête), `edge_ttl.mode: "override_origin"` (ignore délibérément les en-têtes du serveur), TTL 31 jours. Créée le 01/04/2025 — les autres règles du même ruleset référencent toutes des chemins de l'ANCIEN site (`toulouseweb.com/backend/public/slider/*`, `.../news/compressed/*`, `.../api/*`) : cette configuration date du legacy et s'appliquait quand même à toute la refonte, sans lien avec elle.

**Fix** : règle "Cache Everything [Template]" désactivée (`enabled: false`) via l'API Cloudflare (`PATCH /zones/{zone}/rulesets/{id}/rules/{rule}`, jeton API fourni par le client avec permission `zone_settings:edit`) — les autres règles du ruleset (bypass admin, cache images legacy, bypass cinéma/annonces/feed) laissées intactes. Cloudflare respecte désormais les en-têtes envoyés par l'application (déjà corrects : `no-cache, private` sur les pages avec session, `Cache-Control` public sur le contenu statique/public). Vérifié en direct après correctif : `cf-cache-status: DYNAMIC` sur `/`, `/contact`, `/annuaire/deposer`, formulaire de contact fonctionnel de bout en bout (302 + messages d'erreur corrects au lieu de 419).

Aucune modification de code — configuration Cloudflare uniquement. Documenté ici pour qu'un futur repli ne recrée pas cette règle par erreur.

## 34. Suivi des clics (stats admin) ne s'enregistrait jamais + filtrage par date (12/09/2026, demande client)

Demande client : *"dans l'admin dashboard, les stats ne semblent pas fonctionner car j'ai navigué sur plusieurs pages et aucun clic [ne s'enregistre] : tous les liens (pages) navigués doivent s'enregistrer dans les stats. Aussi ajoute un filtrage par date."*

### ⚠️ Bug réel trouvé et corrigé : `sendBeacon()` ne peut pas envoyer le jeton CSRF

`resources/js/track-click.js` utilise `navigator.sendBeacon()` en priorité dès qu'il est disponible (la quasi-totalité des navigateurs modernes) — cette API ne permet PAS de définir d'en-tête personnalisé, donc jamais de `X-CSRF-TOKEN`. Le fallback `fetch()` (qui, lui, envoie bien le jeton) n'est utilisé QUE si `sendBeacon` est indisponible, un cas devenu rarissime en 2026. Vérifié en direct sur la vraie production : `POST /track-click` sans jeton CSRF répond **419** — et `sendBeacon` n'expose jamais l'échec au JavaScript (fire-and-forget par conception), donc chaque clic échouait silencieusement, sans la moindre erreur visible où que ce soit (ni dans la console, ni côté serveur au-delà d'un 419 normal). Explique exactement le symptôme signalé.

**Fix** : route `track-click` exemptée de la vérification CSRF (`bootstrap/app.php`, `$middleware->validateCsrfTokens(except: ['track-click'])`) — un choix standard pour ce type d'endpoint "beacon" (comparable à un webhook) : il ne réalise aucune action privilégiée liée à l'identité du visiteur, se contente d'incrémenter un compteur de clics anonyme (IP/session déjà hachées, voir `ClickTrackingService::record()`) ; `throttle:60,1` (déjà en place sur la route) reste la protection contre l'abus.

**Point de vigilance sur les tests** : Laravel désactive TOUJOURS la vérification CSRF pendant les tests automatisés (`VerifyCsrfToken::runningUnitTests()`) — un simple `$this->post('/track-click')` "réussirait" même SANS cette exemption, et ne prouve donc rien. `tests/Feature/ClickTrackingTest.php` vérifie directement que la route figure dans `ValidateCsrfToken::getExcludedPaths()` (la configuration qui compte réellement en production), pas le comportement d'une requête de test.

### Portée réelle du tracking (clarification, pas un second bug)

Le système ne suit QUE les clics sur des éléments explicitement marqués `data-track="type:id:contexte"` (bannières, cartes de contenu, boutons de réservation/site web...) — pas la simple navigation entre pages générales du menu (Annuaire, Agenda...), qui n'est rattachée à aucune entité précise. C'est le comportement voulu depuis l'origine (brief : statistiques par ENTITÉ, pas des pageviews génériques) — une fois le bug CSRF corrigé, cliquer sur une carte actualité/événement/fiche annuaire s'enregistre bien ; naviguer via le menu principal ne l'a jamais été et ne l'est toujours pas, par conception.

### Filtrage par date du dashboard

Les 3 widgets de clics (`ClicksOverview`, `ClicksByTypeChart`, `TopClickedEntities`) utilisaient des fenêtres de dates codées en dur (aujourd'hui/7j/30j pour le premier, toujours 30 jours pour les deux autres) — aucun moyen de choisir une plage personnalisée.
...
## 50. Sous-rubriques de l'annuaire introuvables — "Restaurants" (19/09/2026, demande client)

Demande client (retour du propriétaire) : *"remets la rubrique Restaurants dans les menus [...] il y a plusieurs sous rubriques [...] il y avait une rubrique 'restaurant spectacle' [...] Un restaurant spectacle doit être en restaurant spectacle et pas en spectacle [...] OU on crée dans la rubrique 'spectacle' une sous rubrique 'restaurant spectacle'"* — analyse demandée du dossier `old/` pour trancher.

### Root cause réelle : PAS une catégorie manquante, une navigation manquante

Vérifié en production : "Restaurants" (id 70, `is_active=1`) et ses **9 sous-catégories** (Afterwork, ambiance, Brasseries Bistrots, `diner_spectacle`, gastronomie, Guinguettes, Livraison à domicile, Pizzerias, traditionnel) existaient déjà en base — import legacy intact, avec 80 fiches publiées au total sous ces sous-catégories (`diner_spectacle` en a déjà 2 : "Cabaret O Toulouse", etc.). Le lien "Restaurants" est même déjà présent dans le header (`resources/views/components/site/header.blade.php`) et `/annuaire/restaurants` répond bien 200 avec des fiches. **Le vrai problème** : `ListingController::index()`/`annuaire/index.blade.php` n'exposaient QUE les catégories de niveau 0 dans la sidebar — aucune page n'affichait les sous-catégories d'une rubrique. Résultat : impossible de naviguer vers "Restaurant spectacle" (ou n'importe quelle autre sous-rubrique) sans en connaître l'URL exacte à l'avance — d'où le ressenti "la rubrique a disparu du menu", alors que la donnée et la page existaient, seule la navigation manquait.

### Analyse `old/` — précédent historique clair pour la structure

Deux schémas legacy trouvés : `t_category` (structure niveau 0/1/2, reprise telle quelle par le `Category` actuel) et `site_client` (table de fiches restaurant plus ancienne, colonne `cat` en texte libre du type `Restau-diner_spectacle`). Les 6 vraies fiches historiques taguées `Restau-diner_spectacle` (Stardust, Cabaret O Toulouse, Le Citron Bleu, Cabaret le Rex, Le Kalinka, Les 3T café-théâtre) étaient **classées sous Restaurants, jamais sous Spectacles** — précédent qui confirme le choix déjà fait par la migration (`diner_spectacle` a `parent_id` = Restaurants, pas Spectacles) et correspond exactement au raisonnement du propriétaire ("on ne sait pas quel spectacle aller voir un jour donné"). **Aucun changement de structure nécessaire** — la donnée était déjà correctement organisée, juste invisible dans la navigation.

### Corrections apportées

1. **Sous-catégories navigables** (`ListingController::index()` + `annuaire/index.blade.php`) : calcul de `$topCategory` (ancêtre niveau 0, en remontant `parent` récursivement) et `$subCategories` (enfants actifs de ce top, PAS seulement de la catégorie courante — pour rester navigable d'une sous-rubrique à l'autre, ex. rester sur "Restaurants" en passant de "Restaurant spectacle" à "Guinguettes"). Rendu en pastilles cliquables (même style que les pastilles catégorie de l'agenda, §46), sous le H1. La sidebar reste en surbrillance sur le TOP niveau (pas seulement en cas d'égalité exacte) même en naviguant dans une sous-rubrique.
2. **Fil d'ariane complet** : remonte désormais toute la chaîne de parents (`Annuaire > Restaurants > Restaurant spectacle`), au lieu de sauter directement de "Annuaire" à la sous-catégorie sans jamais mentionner "Restaurants" — répond directement au 2e volet de la demande ("pour savoir lequel [spectacle] on clique sur le site" — le chemin de retour vers Restaurants est maintenant explicite).
...
- Clés ajoutées en production (21/09/2026, GO client) : `.env` Infomaniak + `config:cache` rebuild. Vérifié en direct : meta `recaptcha-site-key` présente sur les pages publiques.

⚠️ **Bug réel trouvé et corrigé en vérifiant le fonctionnement en production juste après activation** : une soumission `/contact` SANS le champ `recaptcha_token` du tout (testée en direct via `curl`, sans passer par le JS) a été ACCEPTÉE alors qu'elle aurait dû être rejetée. Cause : par défaut, Laravel n'invoque `validate()` d'une règle personnalisée QUE si le champ est présent dans la requête (sauf règle "implicite" comme `required`) — un bot qui omet purement et simplement `recaptcha_token` (plus basique encore que remplir le honeypot) passait intégralement à travers le contrôle. `Illuminate\Contracts\Validation\ImplicitRule` existe pour ce cas mais est lié à l'ancien contrat `Rule` (`passes`/`message`), incompatible avec `ValidationRule` (`validate`/`Closure $fail`) utilisé ici. **Fix retenu** : nouvelle méthode statique `Recaptcha::rules(string $action)` — ajoute `'required'` (règle native Laravel, elle-même implicite) au tableau de règles SEULEMENT quand `services.recaptcha.secret_key` est configurée, suffisant à faire échouer la validation sur un champ absent. Les 7 contrôleurs utilisent maintenant `Recaptcha::rules($action)` au lieu de `[new Recaptcha($action)]` directement.
...
### Découverte en vérifiant en production : les doublons OpenAgenda ne sont PAS un artefact ponctuel

En nettoyant les doublons en production (les deux commandes ci-dessus, déjà déployées au §47/§52), un `content:dedupe-duplicate-events` relancé APRÈS un simple ré-scraping manuel a retrouvé **13 nouveaux groupes de doublons** créés en quelques heures — la preuve qu'OpenAgenda republie RÉELLEMENT le même événement sous un nouveau slug d'un jour sur l'autre (pas une donnée figée héritée de la migration, un phénomène CONTINU côté source). Un nettoyage ponctuel via SSH ne suffit donc pas : `App\Console\Commands\RunWebCron::handle()` (le point d'entrée réel du cron en production, voir §27 — `routes/console.php`/`Schedule::` n'est PAS utilisé en production, hébergement mutualisé sans crontab serveur) appelle désormais `content:dedupe-agenda-manual-entries` et `content:dedupe-duplicate-events` juste après `scrape:events`, à CHAQUE exécution quotidienne. Test : `WebCronTest::test_webcron_run_cleans_up_agenda_duplicates_on_every_run`.

## 56. Adresse par événement étendue à TOUS les scrapers agenda + reset complet du scraping (24/09/2026, demande client)

### 1. Le §55/5 concluait à tort que les 5 drivers "single-venue" n'avaient rien à changer

Demande client explicite : "les adresses ne doivent jamais être les adresses de Areas" — plus large que la conclusion du §55/5 (qui limitait l'extraction à `AbstractOpenAgendaDriver`). Ré-investigation EN DIRECT des 8 drivers restants (pas une supposition) — verdict différent par site, chacun vérifié contre le vrai site/API :

- **`AbstractArdeiSoftDriver` (EscaleDriver/ArdeiDriver)** : la plupart des spectacles ont bien lieu dans la salle elle-même, MAIS une minorité réelle (1/73 constaté en direct pour L'Escale) a lieu dans un lieu satellite explicitement indiqué par l'API elle-même, sous un label `"Lieu : ..."` à l'intérieur du HTML riche de `txt` (ex. "Maison de Quartier de Quéfets, 1 Boulevard Alain Savary 31170 Tournefeuille") — jamais lu jusqu'ici. Nouvelle méthode `extractVenueAddress()`, appliquée sur le texte déjà nettoyé par `cleanRichText()` (regex sur du texte structuré en blocs, plus fiable que sur le HTML brut). Test : `EscaleScraperTest::test_extracts_satellite_venue_address_from_lieu_label`.
- **`TheatreDeLaCiteDriver`** : programme dans plusieurs salles distinctes du même bâtiment ("La Salle", "Le CUB" — constaté en direct sur 5 fiches "spectacle" réelles). Récupéré depuis la ligne `.spectacle__informations__content__line` dont le texte commence par "La "/"Le " (recherche par CONTENU, pas par position : la position de cette ligne varie, et les fiches "evenement" mélangent tout sur une seule ligne au lieu de lignes séparées — dans ce cas le motif ne matche jamais en tête de ligne, aucune extraction erronée). Test : `ScrapeEventsTest::test_extracts_room_name_from_real_spectacle_detail_page`.
- **`OdyssudDriver`** : programme aussi HORS LES MURS, dans des lieux partenaires de VILLES DIFFÉRENTES — constaté en direct : "Basilique Notre-Dame de la Daurade, Toulouse" pour un spectacle réel, contre "Parc d'Odyssud, Blagnac" (l'Area) pour un autre. Le cas le plus flagrant de tous les drivers vérifiés : l'Area générique aurait affiché une VILLE FAUSSE pour cet événement. Extrait depuis `.spectacle--lieu .field--name-name` (nom du lieu) et le 1er `<p>` de `.field--name-description` (ville — le 2e `<p>` n'est qu'un lien "S'y rendre", exclu). Test : `OdyssudScraperTest::test_extracts_off_site_venue_name_and_city`.
- **`BijouDriver`** : l'API Soticket fournit un objet `location` complet par spectacle (adresse, ville, code postal) — identique sur les 44 spectacles courants (Le Bijou n'a qu'une seule salle), mais lu directement depuis l'API plutôt que supposé égal à l'Area, au cas où la programmation utiliserait un jour un lieu partenaire différent. Test : `BijouScraperTest::test_creates_event_from_real_soticket_shape` (assertions `venue_name`/`venue_address` ajoutées).
- **`GaronneDriver` / `GrandRondDriver`** : vérifié en direct sur une vraie fiche détail de chaque site — aucune mention de salle/lieu distincte de l'Area (une seule salle, pas de programmation hors les murs constatée). Rien à changer, conclusion confirmée (pas juste supposée) pour ces deux-là.
- **`LeventDesSignesDriver`** : le seul indice de "lieu" trouvé sur la fiche détail est un gabarit non résolu côté serveur (`{{lieu}}`, rendu côté client par un widget de billetterie) — aucune donnée exploitable sans exécuter le JS du site. Rien à changer.
- **`CasinoBarriereDriver` / `InterpreteDriver`** : limitations déjà documentées au §13 (front Nuxt3 sans lien de détail server-side / CMS remplacé, site actuel non scrapé du tout) — reconfirmées en direct le 24/09/2026, toujours d'actualité, hors budget de ce correctif (0 événement importé par ces 2 drivers actuellement, donc aucune régression d'adresse possible tant qu'ils restent cassés).

### 2. Reset complet des événements scrapés (doublons/incohérences de date persistants)

Demande client : "Supprime tous les scrapings des agendas (NE TOUCHE PAS CELUI DES CINÉMAS NI LES AGENDAS SAISIS MANUELLEMENT) et relance complètement le scraping". Nouvelle commande `content:reset-scraped-agenda-events` (`App\Console\Commands\Migration\ResetScrapedAgendaEvents`), portée strictement `events.source = 'scraped'` — jamais `manual`/`user_submitted`, jamais `Movie`/`Screening`/`ScreeningTime` (cinéma, table séparée).

⚠️ Suppression DÉFINITIVE (`forceDelete()`), pas un soft delete comme les autres commandes de migration de cette phase — choix explicite du client après clarification du compromis technique : `events.slug` a une contrainte UNIQUE en base, et `HasSlug` (spatie/laravel-sluggable) évite les collisions de slug même avec des lignes soft-deleted (`otherRecordExistsWithSlug()` fait un `withTrashed()`, vérifié dans le vendor) — un soft delete aurait donc forcé le nouveau `scrape:events` à générer des slugs suffixés ("-2", "-3"...) pour CHAQUE événement re-scrapé à l'identique, cassant les URLs déjà partagées/indexées pour un simple nettoyage. Le force delete libère le slug d'origine : le nouveau scraping régénère les mêmes URLs propres. Rien n'est réellement perdu : ces données sont 100% reproductibles par le scraping qui suit immédiatement. Test : `ResetScrapedAgendaEventsTest`.
...
Exécution en production le 25/09/2026 : `content:reset-scraped-agenda-events` a supprimé définitivement 1216 événements `source=scraped`, suivi d'un `scrape:events` complet (12 sources actives). ⚠️ La connexion SSH a été interrompue ("Network error: Software caused connection abort") après les 5 premières sources (Théâtre de la Cité/Zénith/Métropole/Casino/Garonne) — relancé source par source (`--source=ID`) pour les 7 restantes, chacune déjà repeuplée entre-temps (créés=0/mis à jour=N) par le WebCron quotidien automatique (08h00 Paris, voir §27) qui s'est apparemment déclenché dans le même créneau — sans risque de doublon grâce à l'upsert par `external_ref`. État final vérifié : 789 lignes `source=scraped` en base (710 non supprimées après un nouveau passage de dédoublonnage — 11 groupes trouvés, comme prévu vu le §55, phénomène continu côté OpenAgenda), 515/789 avec `venue_address` renseigné, `manual`/`user_submitted` inchangés (18773/18). Vérifié en direct : `/agenda/je-suis-chanteur-a-lopera-recital-lyrique` affiche bien "Maison de Quartier de Quéfets, 1 Boulevard Alain Savary 31170 Tournefeuille" (JSON-LD inclus) au lieu de l'adresse générique de L'Escale.
...
Exécuté en production le 25/09/2026 : 9697 lignes supprimées définitivement, `scrape:events` relancé source par source (12 sources, même précaution qu'au §56 après une nouvelle coupure réseau ponctuelle sur la première tentative globale), puis les 2 commandes de dédoublonnage (11 doublons trouvés et supprimés, comme au §56 — phénomène continu côté OpenAgenda). Vérifié en direct :
- `/agenda/exposition-air-france-une-histoire-delegance-1` (recréé frais, `source=scraped`) affiche désormais "L'Envol des Pionniers — 6 Rue Jacqueline Auriol, 31400 Toulouse" (JSON-LD inclus) au lieu de l'adresse générique "Toulouse Métropole".
